...
col.percent.cost <- colorRampPalette(c('red', 'white', 'blue', 'darkblue'))
...
lab23_cost <- textGrob('$ Saved from Base Case (MLE)',
                  gp=gpar(fontfamily=font), rot=270, x=-.25)
...
#= Plot Oxygen Demand
png('code/figures/O2 Demand.png', width=width, height=height, units='in', res=750, bg='transparent')
fig.no <- 2 
for (i in 1:(length(result)-1)) {
  ...
  p[[i]] <- levelplot(O2.demand ~ Nitrogen * Carbon, data=d,
                      ...
                      colorkey = list(labels=list(cex=1, 
                                                  labels=col.labels, 
                                                  par.settings=list(fontfamily=font))))
...
lab14 <- textGrob('Oxygen Demand w/ respect to Base Case (MLE)', gp=gpar(cex=1.25, fontfamily=font))
...
png('code/figures/Sludge Production.png', width=width, height=height, units='in', res=750, bg='transparent')
fig.no <- 3
for (i in 1:(length(result)-1)) {
...
# Assign Oxygen Demand graphs to layout
lab14 <- textGrob('Sludge Production w/ respect to Base Case (MLE)', gp=gpar(cex=1.25, fontfamily=font))
lab1 <- textGrob(paste(paste(fig.no, fig.labels[1], sep=''), lab1.name), 
                 gp=gpar(fontfamily=font))
lab2 <- textGrob(paste(paste(fig.no, fig.labels[2], sep=''), lab2.name),
                 gp=gpar(fontfamily=font))
...
png('code/figures/Methane Production.png', width=width, height=height, units='in', res=500, bg='transparent')
fig.no <- 4
for (i in 1:(length(result)-1)) {
...
lab14 <- textGrob('Methane Production w/ respect to Base Case (MLE)', gp=gpar(cex=1.25, fontfamily=font))
lab1 <- textGrob(paste(paste(fig.no, fig.labels[1], sep=''), lab1.name), 
                 gp=gpar(fontfamily=font))
lab2 <- textGrob(paste(paste(fig.no, fig.labels[2], sep=''), lab2.name),
                 gp=gpar(fontfamily=font))
...
#= Plot Cost 
png('code/figures/Cost.png', width=width, height=height, units='in', res=750, bg='transparent')
fig.no <- 2 
for (i in 1:(length(result)-1)) {
  d <- select(result[[i+1]], Nitrogen, Carbon, cost)
  p[[i]] <- levelplot(cost ~ Nitrogen * Carbon, data=d,
                      panel = function(...){
                        panel.levelplot(...)
                        panel.abline(v = seq(0,Nmax-1, by=15), alpha=0.5)
                        panel.abline(h = seq(0,CODmax-1, by=50), alpha=0.5)
                      }, at=c(seq(-1000,0,length=100),seq(1,5000,length=100),seq(5001,30000, length=100)), col.regions=col.percent.cost,
                      xlab="", ylab="",
                      scales=list(cex=1, tck = c(1,0), 
                                  x=xs,
                                  y=ys), 
                      colorkey = list(labels=list(cex=1,
                                                  labels=c('-$1000', '$0', '$5000', '$10000', '$15000', 
                                                           '$20000', '$25000', '$30000'))))
...
# Assign Cost graphs to layout
lab14 <- textGrob('Total Reduction in Primary Cost Factors from the Base Case (MLE)', gp=gpar(cex=1.25, fontfamily=font))
lab1 <- textGrob(paste(paste(fig.no, fig.labels[1], sep=''), lab1.name), 
                 gp=gpar(fontfamily=font))
lab2 <- textGrob(paste(paste(fig.no, fig.labels[2], sep=''), lab2.name),
                 gp=gpar(fontfamily=font))
...
#= Plot GHG Emissions
png('code/figures/GHG.png', width=width, height=height, units='in', res=500, bg='transparent')
fig.no <- 5
for (i in 1:(length(result)-1)) {
...
lab14 <- textGrob('GHG Production w/ respect to Base Case (MLE)', gp=gpar(cex=1.25, fontfamily=font))
lab1 <- textGrob(paste(paste(fig.no, fig.labels[1], sep=''), lab1.name), 
                 gp=gpar(fontfamily=font))
lab2 <- textGrob(paste(paste(fig.no, fig.labels[2], sep=''), lab2.name),
                 gp=gpar(fontfamily=font))
