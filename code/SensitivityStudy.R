...
plot_ss <- function(df, lab.name, y.ax.scale, pos.neg=FALSE){
  if (pos.neg){
    df$kgCO2.d <- -df$kgCO2.d}
  ggplot(df, aes(y=kgCO2.d, x=e_base/90, 
    ...
    theme(text=element_text(family=font, size=12, color='black'),
...
b_plot_GHG <- plot_ss(result_SS_CO2_B, labB.name, c(-20000,50000), pos.neg=TRUE)
c_plot_GHG <- plot_ss(result_SS_CO2_C, labC.name, c(-20000,50000), pos.neg=TRUE)
d_plot_GHG <- plot_ss(result_SS_CO2_D, labD.name, c(-20000,50000), pos.neg=TRUE)
e_plot_GHG <- plot_ss(result_SS_CO2_E, labE.name, c(-20000,50000), pos.neg=TRUE)
...
ggsave('code/figures/ss_GHG.png', grid.arrange(b_plot_GHG + theme(legend.position="none"), 
                                  ...
                                  bottom=textGrob('Approx. % Change from Conventional System', 
                                                  gp=gpar(fontfamily=font)),
                                  layout_matrix=matrix(c(1,2,1,2,1,2,1,2,1,2,1,2,
                                                         3,4,3,4,3,4,3,4,3,4,3,4,5,5), ncol=2, byrow = TRUE)), width=7, height=6)


b_plot_cost <- plot_ss(result_SS_cost_B, labB.name, c(-5000, 10000))
c_plot_cost <- plot_ss(result_SS_cost_C, labC.name, c(-5000, 10000))
d_plot_cost <- plot_ss(result_SS_cost_D, labD.name, c(-5000, 10000))
e_plot_cost <- plot_ss(result_SS_cost_E, labE.name, c(-5000, 10000))
