...
```
The following document is intended to describe in the models created for the four carbon and nitrogen removal scenarios (Modified Luzak-Ettinger, CANON, anammox-n-damo, and anammox-n-damo-AnMBR):
![Figure 1](code/figures/Scenarios.pdf)
...
- Centrate from the anaerobic digester is returned to the mainstream from treatment and is assumed to contain 25% of the total nitrogen load. [1]
...
*Z* represents a constituent (N for total nitrogen, COD for chemical oxygen demand, O2 for oxygen, etc.) and *Y* represents an organism (AOB for ammonium oxidizing bacteria, NOB for nitite oxidizing bacteria, etc.)
...
| ---------------------------------- | ----------------------- | ------------------- |
| Heterotrophic Denitrification [1]  | 5      gCOD/gN          | 0.32  gVSS/gCOD     |
...
Y_HET <<- 0.45/n_conv      # gCOD/gCOD heterotrophs
...
$$L_N = L_{N_{in}}+L_{N_{cent}}$$
  Where:
$$L_{N_{in}}= C_{N_{in}}\times Q=40 \frac{ML}{day}\times40\frac{kg}{ML}=1600$$
```{r}
(LNin <- Q * cNin) #kgN/d, Nitrogen load per day
```
  
  And where total nitrogen load in the centrate is assumed to be 25% of the total load in the influent, as is typical of wastewater centrate [1].
$$L_{N_{cent}}= C_{N_{in}}\times 0.25$$
  
  
  Total COD load into the system for all scenarios is determined as follows:
$$L_{COD_{in}}= C_{COD_{in}}\times Q$$
```{r}
  (LNin <- Q * cNin) #kgN/d, Nitrogen load per day
  (LNcent <- .25 * LNin) #kgN/d, Nitrogen load from centrate, assumed 40% of total load
...
$$COD_{req'd}=L_{N}\times S_{COD_{DENIT}}$$
...
```{r}
(COD.reqd <- LN * sCOD_DENIT) # ammt of COD req'd.
(COD.bal <- COD_reqd - LCOD)
if (COD.bal > 0){
  COD.added <- COD.bal
}else {
  COD.added <- 0
...
It was found that there was enough COD for conversion at all COD/N ratios simulated.
