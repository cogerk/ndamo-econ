---
title: "SI 1. Sample Calculations"
output: pdf_document
---
...
These calculations are intended to explain the R scripts used to create the figures **xx-yy**

### Definitions & Input Parameter
...
$f_{x_Y}$, Sludge production of organism *Y* 

| Process                       | $S_{Z_{Y}}$           | $f_{x_Y}$           |
| ----------------------------- | --------------------- | ------------------- |
| Heterotrophic Denitrification | 5      gCOD/gN        | 0.56  gCOD/gCOD     |
| Heterotrophic Oxidation       | 1      g$O_2$/gCOD    | 0.56  gCOD/gCOD     |
| Ammonium Oxidation            | 1.5    mol$O_2$/molN  | 0.20  gCOD/gN       |
| Nitrite Oxidation             | 1      mol$O_2$/molN  | 0.06  gCOD/gN       |
| Anammox Bacteria              | 1.5    mol$O_2$/molN  | 0.20  gCOD/gN       |
| Anaerobic Membrane            | N/A                   | 0.56  gCOD/gCOD     |
| n-DAMO                        | 0.338  mol$CH_4$/molN | 0.22  gCOD/g$CH_4$  |

$n=1.4$, gVSS/gCOD, conversion constant **[cite]**

$f_{COD/HET}=5$, gCOD/gN, gram COD required per gram of denitrified nitrogen **[cite]**
...
#### Assumed Constants  
$f_{x_{digester}}=0.59$, Assumed digester sludge reduction

$f_{CH_4/COD}=0.40$, m$^3$/kg, m$^3$ of methane gas produced per kg of COD at 35 °C in an anaerobic digester **[cite]**
...
$$COD_{req'd}=L_{N}\times f_{COD/HET}$$
...
$$p_{x_{HET}} = COD_{req'd}\times n \times f_{x_{HET}} $$
```{r}
...
It is assumed that nitrifcation occurs at a ratio that is ideal for anammox removal, that is:
$$f_{N_{AOB}} = 0.57$$
$$f_{N_{NOB}} = 0$$
**[see appendix]**
The fraction of nitrogen as ammonium consumed by anammox is therefore 
$$f_{NH_{4_{AMX}}} = (1 - f_{N_{AOB}})\times2\times1.02$$
...
** i. Nitrifcation **
48.5% of flow was calculated to be the appropriate diversion to ensure 100% N conversion. **[see appendix]** Unlike the CANON system, The nitrification system is assumed to have 100% nitratation by NOB.

$$f_{N_{AOB}} = f_{N_{NOB}} = 1 - 0.485$$
...
#### Step 4a,b & c) Anaerobic Digester Calculations
  ...
  A typical anaerobic digester sludge reduction factor $f_{x_{digester}} = $ is used to calculate actual sludge out of the system requiring handling:
$$p_{x_{out}}=p_{x_{TOT}}\times(1-f_{x_{digester}})$$
	The COD of the digested sludge is converted to methane:
$$CH_{4_{prod}} =  \frac{p_{x_{TOT}}-p_{x_{out}}}{n}\times f_{CH_4/COD}$$
```{r}
  (px_out <- px_TOT * (1 - fx_digester))
  (CH4prod <- (px_TOT - px_out)/n * f_CH4_COD) #m3 CH4 produced/day calculated as a mass balance
